...
export const MaintenanceHistoryPage = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const data = await api.get('/history/maintenance');
        setHistory(data);
      } catch (err) {
        addToast(err.message || 'Failed to fetch maintenance history', 'error');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="fade-in" style={{ paddingBottom: '40px' }}>
      <h2 style={{ marginBottom: '24px', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        Maintenance & Repair History Log
      </h2>
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th>Plant Name</th>
                  <th>Unit #</th>
                  <th>Fault Repaired</th>
                  <th>Original Confidence</th>
                  <th>Date Repaired & Returned</th>
                  <th>Status</th>
                </tr>
              </thead>
              <tbody>
                {history.map((log) => (
                  <tr key={log.id}>
                    ...
                    <td className="text-emerald" style={{ fontWeight: 500 }}>{log.faultType}</td>
                    ...
                    <td>
                      <span className="status-badge status-running">Running / Healthy</span>
                    </td>
                  </tr>
                ))}
              </tbody>
